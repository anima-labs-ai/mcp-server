...
/**
 * Scopes advertised to MCP clients as RFC 9728 `scopes_supported`.
 *
 * Anima Connect rejects an authorization request that carries no `scope`, so a
 * client which cannot discover these sends none and the flow dies at
 * `invalid_request`. Advertising them is what makes a third-party OAuth
 * connection possible at all.
 *
 * Deliberately narrower than the authorization server's own list:
 *   - `admin:full` is a privilege escalation no MCP tool call needs, and
 *     advertising it invites clients to request it by default.
 *   - `vault:read_credential:{label}` is a template, not a literal scope; a
 *     client cannot send it without substituting a label.
 * Override with MCP_OAUTH_SCOPES (comma-separated) to narrow further.
 */
