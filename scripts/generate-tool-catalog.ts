...
function renderToolEntry(tool: Tool): string {
	...
	lines.push("");
	// File path only — a :line suffix would shift on any edit to the file,
	// churning the committed catalog on every unrelated change.
...
async function main() {
	...
	// No "Last generated" date and no line numbers anywhere in the output:
	// regeneration must be byte-stable when no tool actually changed, so the
	// committed catalog only diffs on real tool changes (git history already
